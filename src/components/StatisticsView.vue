...
// generated by Codex
const exportPNG = () => {
  const svgEl = document.querySelector<SVGSVGElement>('#karps_graph svg')
  if (svgEl) {
    const bbox = svgEl.getBBox()
    const width = bbox.width + 50
    const height = bbox.height + 50 // add some padding, else labels get cut off
    ...
    if (ctx) {
      ...
      const svgBlob = new Blob([`<svg xmlns="http://www.w3.org/2000/svg">${svgData}</svg>`], {
      ...
      img.onload = () => {
        ...
        canvas.toBlob((blob) => {
          if (blob) {
            ...
            downloadLink.download = 'overview.png'
            downloadLink.click()
